...
public sealed partial class JsonhReader : IDisposable {
    ...
    /// <summary>
    /// Tries to find the place to insert the given property name in the source.<br/>
    ...
    ///     "c": "2"
    ///   },
    ///   /* Start range */"c": "3"/* End range */
    /// }
    /// </code>
    /// </summary>
    public static Result<string> InsertProperty(string Source, string PropertyName, scoped ReadOnlySpan<char> RawProperty, scoped ReadOnlySpan<char> RawPropertyValue, scoped ReadOnlySpan<char> Indent) {
        long CurrentDepth = 0;
        ...
        foreach (Result<JsonhToken> TokenResult in Reader.ReadElement()) {
            ...
            switch (Token.JsonType) {
                ...
                case JsonTokenType.EndObject: {
                    // Insert found
                    if (CurrentDepth == 1) {
                        int StartEndObjectIndex = (int)StartTokenCharCounter;

                        // Insert property
                        return string.Concat(
                            Source.AsSpan(..StartEndObjectIndex),
                            string.Concat(
                                PropertyCount == 0 ? "\n" : "",
                                Indent
                            ),
                ...
                case JsonTokenType.PropertyName: {
                    foreach (Result<JsonhToken> _ in Reader.ReadCommentsAndWhitespace()) {
                    }
                    int StartPropertyValueIndex = (int)Reader.CharCounter;

                    // Replace found
                    if (CurrentDepth == 1 && Token.Value.Equals(PropertyName, StringComparison.Ordinal)) {
                        // Read property value
                        foreach (Result<JsonhToken> _ in Reader.ReadElement()) {
                        }
                        int EndPropertyValueIndex = (int)Reader.CharCounter;

                        // Replace property
                        return string.Concat(
                            Source.AsSpan(..StartPropertyValueIndex),
                            RawPropertyValue,
                            Source.AsSpan(EndPropertyValueIndex..)
                        );
                    }
        ...
        // Path not found
        return new Error("Place to insert property not found");
    }
